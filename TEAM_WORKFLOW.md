...
| 2026-05-11 | Started backend chat persistence foundation on branch `codex/backend-chat-persistence` with DB-backed sessions/messages and a `GET /sessions` API | Chat history must move off in-memory storage before mobile can migrate away from SharedPreferences | `python3 -m py_compile services/agent/*.py`; sqlite smoke test for create/read/delete session + messages | services/agent/db.py, services/agent/agent.py, services/agent/main.py, TEAM_WORKFLOW.md, AGENTS.md, CLAUDE.md |
| 2026-07-07 | Fixed all RenderFlex overflow risks across 24 screens: unwrapped Row texts got Expanded/Flexible + ellipsis, chip Rows became Wrap/horizontal-scroll, fixed-height cards became content-driven (minHeight/IntrinsicHeight), Figma-absolute Positioned coords re-anchored to right/bottom, chat header PreferredSize now includes status-bar inset, login form made keyboard-scrollable | User reported "overflowed by 51 pixels" and "bottom overflowed by 2 pixels" stripes on device; systematic audit found the same fixed-size-vs-scaling-text root cause in every screen | `flutter analyze` — 0 issues; pixel math verified per finding at 360-430dp and textScale 1.0-1.3 | 24 files under mobile/app/lib/screens/ (see CLAUDE.md changelog) |
