...
| 2026-05-12 | Full pixel-perfect home screen rebuild — iOS-style AppBar (TVK logo + language + avatar), Campaign Song card (hardcoded YouTube JHJmFbLeK-Y, full-screen player on tap), Latest Updates horizontal scroll 5 cards with tap→NewsDetailScreen, Know Your Leaders 2×2 grid tap→LeaderScreen, Social Justice PageView carousel, Community Hall full section with live posts, Campaign Toolkit Figma-exact 2×2 gradient grid (Posters/Media/Slogans/Hashtags with rotated thumbnail cards), sections ordered per Figma. LeaderScreen + NewsDetailScreen given iOS-style AppBar + back button. | mobile/app/lib/screens/home_screen.dart, mobile/app/lib/screens/leader_screen.dart, mobile/app/lib/screens/news_detail_screen.dart |
| 2026-05-13 | Pixel-perfect hero section rebuild from Figma node 1328:4546 — removed Scaffold.appBar entirely; dark brownish-red gradient hero (#3D0A0A→#6B1212) covers full screen from y=0; blurred tvk_flag_bg at 40% opacity; two red ellipse RadialGradient glows; large Vijay (white shirt) x=94 y=-155; transparent app bar overlay at topPad with white icons (TVK logo circle, frosted language, profile); Tamil party name at y=193; subtitle at y=259; _HeroWavePainter (yellow #FFCA00 trapezoid + red #E40101 diagonal split) at y=281; small Vijay at x=-47 y=167; "Join TVK today!" RichText (white+#FFCA00 TVK) at y=379; "Join Now →" white/red button 92×27 at y=424. SystemUiOverlayStyle.light for white status-bar icons. Deleted: homeAppBar(), old _HeroSection, _JoinTvkCard, _SocialJusticeHeroCard, _CommunityWallCard. | mobile/app/lib/screens/home_screen.dart |
| 2026-05-13 | Dashboard pixel-perfect pass with all new Figma assets — hero bg: hero_bg_blur.png (replaces tvk_flag_bg.png), small Vijay: vijay_wave.png (replaces vijay_hero.png); Campaign Song card redesigned as white left-text/right-photo layout (news_update_1.png) with 🔥 New pill + Watch Now button matching Figma 1328:9129; Latest Updates changed from h-scroll to full-width PageView with red dot indicators, card redesigned as 220px photo-bg overlay (news_update_1.png) with title+subtitle overlay matching Figma 1328:9138/9144; Manifesto banner uses vijay_manifesto.png; TVK Videos fallback replaced with Figma-exact TVK Television banner (red gradient, tv_flat_screen.png + vijay_tv_screenshot.png + tv_wooden_table.png + text); Campaign Toolkit cards now use tvk_vijay_thalaiva_1.png and tvk_vijay_thalaiva_2.png as rotated thumbnails. Zero analyzer issues. | mobile/app/lib/screens/home_screen.dart |
